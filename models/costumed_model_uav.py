...
class StackedCNNEncoderWithPooling(nn.Module):
    def __init__(self, num_classes, zero_init_residual=False, input_channels=1, hidden_size=32, num_blocks=10, pooling="max"):
        """
        ...
        for i in range(num_blocks):
            ...
            # Add pooling layer
            if pooling == "avg":
                layers.append(nn.AvgPool1d(kernel_size=2, stride=1))  # Average pooling
            elif pooling == "max":
                layers.append(nn.MaxPool1d(kernel_size=2, stride=1))  # Max pooling
        layers.append(Flatten())
        self.encoder = nn.Sequential(*layers)

        # Add a dummy pass to compute the final output size
        with torch.no_grad():
            dummy_input = torch.zeros(1, input_channels, 69)  # Example input size
            output_size = self.encoder(dummy_input).view(1, -1).size(1)
            print(f"Calculated output size after encoder: {output_size}")

        # Now adjust adaptive_fc based on calculated output size
        self.adaptive_fc = nn.Sequential(
            nn.Linear(output_size, 256),  # Use calculated output_size
            nn.BatchNorm1d(256),
            nn.ReLU()
        )

        # Ensure that the input size of the following fully connected layer is 256
    ...
    def forward(self, x):
        ...
        """
        x = self.encoder(x)
        print(f"Shape after encoder: {x.shape}")  # Print the shape after encoder
        x = self.adaptive_fc(x)
        print(f"Shape after adaptive_fc: {x.shape}")  # Print the shape after adaptive_fc
        x = self.fc(x)
        print(f"Shape after fc: {x.shape}")  # Print the shape after fc
