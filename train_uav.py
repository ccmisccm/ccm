...
from utils_uav import check_accuracy, check_class_accuracy,check_accuracy_final
import torch
...
def train(model, tr_dataset, val_dataset, args,ssv_method,aug_method,loop_flag=False):
    epochs = args.epochs
    ...
    from models.costumed_model_uav import ClassBalancedLoss
    ...
    if loop_flag==True:
        print('保存的函数被调用几次')
        val_acc, _= check_accuracy_final(model, val_loader, device,ssv_method,aug_method, error_analysis=True,filename="fine_tune")
    else:
        val_acc, _ = check_accuracy(model, val_loader, device, error_analysis=True)
    # 在训练结束后，调用混淆矩阵绘制函数，评估验证集分类效果
