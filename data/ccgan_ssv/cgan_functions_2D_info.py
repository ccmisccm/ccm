...
def gradient_penalty(y, x, args):
    ...
    dydx = torch.autograd.grad(outputs=y,
                               ...
                               create_graph=True,
                               only_inputs=True,
                               allow_unused=True)[0]  # Adding allow_unused=True

    if dydx is None:
        return torch.tensor(0.0).cuda(args.gpu, non_blocking=True)  # In case dydx is None
...
def mutual_information_loss(cat_logits, con_params, true_cat, true_con, lambda_cat=1.0, lambda_con=0.1):
    """
    计算 InfoGAN 中的互信息损失
    对于离散变量使用交叉熵损失，对于连续变量使用 L2 损失（均方误差）。
    """
    # 对离散变量计算交叉熵损失
    ce_loss = F.cross_entropy(cat_logits, true_cat)
    # 对连续变量计算均方误差损失
    mse_loss = F.mse_loss(con_params, true_con)
    return lambda_cat * ce_loss + lambda_con * mse_loss
...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, (real_imgs, real_img_labels) in enumerate(tqdm(train_loader)):
       ...
       #noise = torch.cuda.FloatTensor(np.random.normal(0, 1, (real_imgs.shape[0], args.latent_dim))).cuda(args.gpu, non_blocking=True)
        ...
        if hasattr(gen_net, 'module'):
            cont_dim = gen_net.module.continuous_dim
        else:
            cont_dim = gen_net.continuous_dim
        cont_code = torch.zeros(noise.size(0), cont_dim).to(noise.device)

        fake_imgs = gen_net(noise, fake_img_labels, cont_code)
        ...
        d_cls_loss = cls_criterion(r_out_cls[0], real_img_labels)
        ...
        gen_net.zero_grad()
        cont_code = torch.zeros(noise.size(0), device=noise.device)  # 形状为 [B]，或根据需要构造成 [B, 1]
        gen_imgs = gen_net(noise, fake_img_labels, cont_code)

        g_out_adv, g_out_cls = dis_net(gen_imgs)

        g_adv_loss = -torch.mean(g_out_adv)
        g_cls_loss = cls_criterion(g_out_cls[0], fake_img_labels)

        # 计算互信息损失
        # 假设 dis_net 返回的 f_out_cls 是识别头预测的类别和连续变量
        cat_logits, con_params = f_out_cls  # 识别头输出的类别 logits 和连续变量参数
        true_cat = fake_img_labels  # 真实的类别标签
        # 假设 batch_size = 32，连续隐变量维度为 2
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        true_con = torch.zeros(32, 2, device=device)  # 或根据实际需求采样，比如从均匀分布中采样
        # 真实的连续变量（假设连续变量是噪声的一部分）

        # 计算互信息损失
        mi_loss = mutual_information_loss(cat_logits, con_params, true_cat, true_con, lambda_cat=1.0, lambda_con=0.1)

        # 总生成器损失 = 对抗损失 + 分类损失 + 互信息损失
        g_loss = g_adv_loss + lambda_cls * g_cls_loss + mi_loss
