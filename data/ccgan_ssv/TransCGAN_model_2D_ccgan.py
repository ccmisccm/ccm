...
class Generator(nn.Module):
    def __init__(self, seq_len=32, channels=3, num_classes=9, latent_dim=100, data_embed_dim=3,
                label_embed_dim=10 ,depth=3, num_heads=3,
        ...
        self.l1 = nn.Linear(110, self.seq_len * self.data_embed_dim)
        ...
        self.linear_up = nn.Linear(69, 100 * 69)  # 将每个 69 维的向量映射到 690 维（10*69）

    def forward(self, z, labels):
        print(f"Shape before concatenation: z={z.shape}, labels={labels.shape}")
        c = self.label_embedding(labels).unsqueeze(1)
        c = c.repeat(1, z.shape[1], 1)
        x = torch.cat([z, c], 2)

        #x = x.view(z.size(0), self.seq_len, self.data_embed_dim)

        H, W = 1, self.seq_len
        print('输入到transformer的形状  ', x.shape)
        x = self.blocks(x)
        x = x.reshape(x.shape[0], 1, x.shape[1], x.shape[2])
        output = self.deconv(x.permute(0, 3, 1, 2))
        output = output.view(output.size(0), output.size(1), -1)
        # 通过线性层将每个 69 维向量映射到 690 维
        output = self.linear_up(output)  # 现在 shape 为 [32, 1, 690]
        # 最后 reshape 成 [32, 1, 10, 69]
        output = output.view(output.size(0), 1, 100, 69)
...
class PatchEmbedding_Linear(nn.Module):
    def __init__(self, in_channels = 21, patch_size = 16, emb_size = 100, seq_length = 1024):
        ...
        self.projection = nn.Sequential(
            ...
            nn.Linear(H * in_channels, emb_size)
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, emb_size))
        self.positions = nn.Parameter(torch.randn((seq_length // patch_size) + 1, emb_size))
        #self.positions = nn.Parameter(torch.randn(1871, emb_size))  ###手动修改错误
