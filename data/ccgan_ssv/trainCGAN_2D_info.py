...
import cfg
from DataLoader_2D_uav import *
from TransCGAN_model_2D_info import *
from cgan_functions_2D_info import train, save_samples, LinearLrDecay, load_params, copy_params, cur_stages
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # import network
    gen_net = Generator( noise_dim=90, cat_dim=7, continuous_dim=1, channels=1, feature_dim=64 )

    print(gen_net)
    dis_net = Discriminator(channels=1, cat_dim=7, continuous_dim=2, feature_dim=64)
    print(dis_net)
...
def gen_plot(gen_net, epoch):
    ...
    for i in range(10):
        fake_noise = torch.FloatTensor(np.random.normal(0, 1, (32, 69, 90))).to('cuda')  # Ensure it is on GPU
        fake_label = torch.randint(0, 7, (32,)).to('cuda')  # Ensure it is on GPU
        cont_code = torch.zeros(fake_noise.size(0), 2).to('cuda')  # Assuming 2 is the continuous variable size

        # Now pass all three inputs to the generator
        fake_sigs = gen_net(fake_noise, fake_label, cont_code).to('cpu').detach().numpy()
    ...
    for i in range(2):
        for j in range(5):
            ...
            axs[i, j].title.set_text(synthetic_labels[i * 5 + j].item())  # Convert label to item for display
    buf = io.BytesIO()
