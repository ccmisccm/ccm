...
from data.ccgan_ssv.TransCGAN_model_1D_uav import *
from data.data_process_1D_uav import mitbih_train
...
class syn_mitbih(Dataset):
    def __init__(self,model_path=r'D:\Users\Administrator\Desktop\Semi-Marc-SimSiam-main\Semi-Marc-SimSiam-main\data\ccgan_ssv\logs\1D_ccgan_2025_03_26_17_29_06\Model\checkpoint', n_samples = 2000, seq_len = 69, reshape = False, transform=None):
    ...
    def generate_synthetic_data(self, gen_net, classlabel, n):
        ...
        for _ in range(n_batches):
            fake_noise = torch.cuda.FloatTensor(np.random.normal(0, 1, (batch_size, 100))).to(device)
        ...
        if remainder > 0:
            fake_noise = torch.cuda.FloatTensor(np.random.normal(0, 1, (remainder,100))).to(device)
...
class mixed_mitbih(Dataset):
    def __init__(self, model_path,real_samples = 200, syn_samples = 300, transform=None):
        self.transform = transform
...
if __name__ == "__main__":
    ...
    syn_ec0g = syn_mitbih(n_samples=800, reshape=True)