...
from utils_uav import check_accuracy_final
import math
import random
...
def marc(model, tr_dataset, val_dataset, args,ssv_method,aug_method,loop_flag=False):
    ...
    from models.costumed_model import ClassBalancedLoss
    loss_fn = ClassBalancedLoss(class_counts)      ##对每个类别的损失进行加权
    ...
    if loop_flag == True:
        val_acc, _ = check_accuracy_final(model, val_loader, device,ssv_method,aug_method,error_analysis=True, filename="marc")
    else:
        val_acc, _ = check_accuracy(model, val_loader, device)
